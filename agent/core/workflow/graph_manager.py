...
from agents.chitchat_agent import ChitchatAgentNode
...
class AgentGraphManager:
    ...
    支持 OOD（Out-of-Domain）路由分支，由 ChitchatAgent 承接无关问题。

    持久化说明（P1 优化）：
    - 引入 LangGraph 原生 MemorySaver checkpointer，实现 state 自动持久化
    - 按 thread_id（user_id:session_id）隔离会话，支持中断恢复
    - 短期对话历史由 checkpointer 自动管理，无需手动 Redis 读写
    - MemorySaver 为内存版（进程级），重启丢失；生产环境可替换为
      PostgresSaver / RedisSaver 实现跨进程持久化
    """
    def __init__(self):
        ...
        self.chitchat_node = ChitchatAgentNode()
        # checkpointer 单例：整个应用共享一个 MemorySaver
        # 所有 thread 的 state 都存在内存中，按 thread_id 隔离
    ...
    def build_graph(self) -> StateGraph:
        ...
        builder.add_node("out_of_scope", self.chitchat_node)
        ...
        # 传入 checkpointer：LangGraph 会自动持久化整个 state（messages/next_agent/metadata）
        # 调用方需在 config["configurable"]["thread_id"] 中指定会话标识

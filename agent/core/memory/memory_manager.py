...
class MemoryManager:
    ...
    def __init__(
        ...
        self.short_term = ShortTermMemory(
            redis_url=redis_url,
            ttl=redis_ttl,
            llm=summary_llm,  # 注入 LLM 用于短期记忆摘要压缩
        )
    ...
    async def background_extract(
        ...
        try:
            ...
            # P1-6 根因修复：去重必须读全量已存条目，不能用语义检索 top-k。
            # load_preferences 用 retrieve_relevant(query="用户偏好习惯个性特点", top_k=3)，
            # 预存条目与该 query 语义相似度不够时召回不到 → existing 为空 → 去重失效 → 重复写入。
            # 改用 list_all（标量过滤全量返回）确保读到全部已存条目。
            # 先 flush 确保上一轮 background_extract 的写入已可见（flush 是异步的，
            # 上一轮结束时调用 flush 只是提交请求，本轮读取前再 flush 一次兜底）。
            self.long_term.flush()
            existing = await self.long_term.list_all(user_id)
            # P1-6 修复：existing_normalized 只保留 "类别|内容"，不带 PREF/FACT 前缀。
            # 原因：_parse_stored_memory 无法从 content "[类别] 内容" 判断是 preference 还是 fact
            #（类型存在 Milvus 的 memory_type 字段，不在 content 里），保守返回 "preference"。
            # 若带前缀，fact 条目会被转成 "PREF|..."，与 extractor 返回的 "FACT|..." 前缀不匹配，
            # 子串去重失效。不带前缀时，"类别|内容" 是 "PREF|类别|内容" 和 "FACT|类别|内容" 的子串，
            # 子串去重能正确命中。
            existing_normalized = []
            for ex in existing:
                parsed_ex = self._parse_stored_memory(ex)
                if parsed_ex:
                    mem_type, category, content = parsed_ex
                    existing_normalized.append(f"{category}|{content}")
            new_items = await extractor.extract(
                conversation_text=conversation_text,
                existing=existing_normalized,
            )
            saved_count = 0
            for item in new_items:
                # 解析 "PREF|类别|内容" 或 "FACT|类别|内容" 格式
                parsed = self._parse_memory_item(item)
                if parsed:
                    ...
                    saved_count += 1
            if saved_count > 0:
                # P1-6 根因修复：批量写入后强制 flush，确保下一轮 list_all 能读到本轮写入。
                # 否则连续两次 background_extract 间数据未 flush，第二次去重失效导致重复累积。
                self.long_term.flush()
                logger.info(
                    "[MEMORY] Background extract: saved %d new items for user '%s'",
                    saved_count, user_id,
                )
                return new_items
            else:
    ...
    async def finalize_session(
        ...
        if self.long_term.available:
            extractor = PreferenceExtractor(llm=llm)
            # P1-6 根因修复：去重用 list_all 全量读取，不用 load_preferences 语义检索 top-k
            # 先 flush 确保上一轮写入可见（同 background_extract）
            self.long_term.flush()
            existing = await self.long_term.list_all(user_id)
            # P1-6 修复：existing_normalized 只保留 "类别|内容"（同 background_extract）
            existing_normalized = []
            for ex in existing:
                parsed_ex = self._parse_stored_memory(ex)
                if parsed_ex:
            ...
            logger.debug(
                ...
                len(existing_normalized), existing_normalized,
            )
            new_items = await extractor.extract(
                conversation_text=conversation_text,
                existing=existing_normalized,
            ...
            # 4. Persist new items to Milvus (解析 PREF|类别|内容 / FACT|类别|内容)
            saved_count = 0
            for item in new_items:
                parsed = self._parse_memory_item(item)
                if parsed:
            ...
            if saved_count > 0:
                # P1-6 根因修复：批量写入后强制 flush（同 background_extract）
                self.long_term.flush()
                logger.info(
                    "[MEMORY] Saved %d new items (pref+fact) for user '%s': %s",
                    saved_count, user_id, new_items,
                )
            else:
                logger.info("[MEMORY] No new items found for user '%s'", user_id)
        else:
    ...
    @staticmethod
    def _parse_memory_item(item: str) -> tuple[str, str, str] | None:
        """解析 'PREF|类别|内容' 格式的记忆条目。

        Returns:
            (memory_type, category, content) 或 None（格式不合法时）
            memory_type: "preference" 或 "fact"
        """
        parts = item.split("|", 2)
        if len(parts) != 3:
            return None
        prefix, category, content = parts
        prefix = prefix.strip().upper()
        if prefix == "PREF":
            return ("preference", category.strip(), content.strip())
        elif prefix == "FACT":
            return ("fact", category.strip(), content.strip())
        return None

    @staticmethod
    def _parse_stored_memory(stored: str) -> tuple[str, str, str] | None:
        """解析 Milvus 中存储的记忆格式 "[类别] 内容"。

        P1-6 修复：存储格式与提取格式不一致，去重前需要统一解析。
        存储的 memory_type 信息在 Milvus 的 memory_type 字段里（不在 content 里），
        这里仅按 content 反推：无法从 content 区分 preference/fact，
        所以返回 memory_type="preference" 作为保守默认值（去重时类别相同即可命中）。
        ...
        if s.startswith("[") and "]" in s:
            # 新格式: "[类别] 内容"
            bracket_end = s.index("]")
            category = s[1:bracket_end].strip()
            content = s[bracket_end + 1:].strip()
            if category and content:
                # 保守返回 preference（去重时类别匹配即可，类型不关键）
                return ("preference", category, content)

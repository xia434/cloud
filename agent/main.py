...
async def run_interactive_mode(
    ...
    try:
        while True:
            ...
            # 4. 定期触发长期内存提取（P0-1 修复：原调用不存在的 extract_and_save_preferences）
            turn_count += 1
            if turn_count % 5 == 0:
                print("🔄 [Background] Triggering long-term memory extraction...")
                asyncio.create_task(
                    memory.background_extract(user_id, session_id, llm=summary_llm)
                )
        ...
        await memory.finalize_session(user_id, session_id, llm=summary_llm)
...
async def main() -> None:
    ...
    # 初始化内存管理器
    # 注入 summary_llm 用于短期记忆摘要压缩（P2-5 修复：否则 CLI 模式压缩永远降级为硬截断）
    ...
    try:
        if args.query:
            ...
            await run_interactive_mode(graph_manager, user_id, session_id, memory, summary_llm)
    finally:
        await memory.close()

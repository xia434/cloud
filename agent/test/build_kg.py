...
# ==========================================
# 4. 主函数入口：支持单文件 / 目录批量导入
# ==========================================
if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(
        description="知识图谱构建工具：从 Markdown 文档提取实体关系并导入 Neo4j",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例：
  # 批量导入 mock_data 下所有 .md（默认）
  python test/build_kg.py

  # 导入单个文件
  python test/build_kg.py mock_data/ecs_product_info.md

  # 导入指定目录
  python test/build_kg.py --dir mock_data
        """,
    )
    parser.add_argument(
    ...
    if args.path:
        ...
        if not files_to_process:
            print(f"❌ 目录中没有 .md 文件: {data_dir}")
            sys.exit(1)

    print(f"📋 共 {len(files_to_process)} 个文档待处理:")
    for f in files_to_process:
        print(f"   - {os.path.basename(f)}")
    print()

    # 逐个处理：提取 KG → 合并 → 导入 Neo4j
    # 注意：每个文件提取后立即导入，利用 MERGE 机制实现增量合并
    all_kg_data = {"nodes": [], "edges": []}
    for i, md_file in enumerate(files_to_process, 1):
        print(f"\n{'='*60}")
        print(f"[{i}/{len(files_to_process)}] 处理: {os.path.basename(md_file)}")
        print(f"{'='*60}")
        try:
            kg_data = extract_knowledge_graph(md_file)
            all_kg_data["nodes"].extend(kg_data["nodes"])
            all_kg_data["edges"].extend(kg_data["edges"])
            # 逐文件导入，利用 MERGE 去重
    ...
    # 汇总
    print(f"\n{'='*60}")
    print(f"🎉 全部完成！共处理 {len(files_to_process)} 个文档")
    print(f"   累计提取节点: {len(all_kg_data['nodes'])}")
    print(f"   累计提取关系: {len(all_kg_data['edges'])}")
    print(f"   （实际入库量受 MERGE 去重影响，可能小于累计值）")
    print(f"{'='*60}")

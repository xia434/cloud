...
def query_vector_db(query: str) -> str:
    ...
    try:
        ...
        candidate_count = max(1, int(os.getenv("RAG_RETRIEVAL_CANDIDATES", "8")))
        final_count = max(1, int(os.getenv("RAG_RETRIEVAL_TOP_K", "3")))
        candidates = store.similarity_search_with_score(query, k=candidate_count)
        results = rerank_results(query, candidates, limit=final_count)
